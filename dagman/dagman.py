...
class DAGManJobCreatorCompact(BaseJobCreator):
    """
    DAGManJobCreatorCompact

    Create DAGMan job files that can be directly submitted:

    - Single shell script with single placeholders for all the job args.
    - Single submit file with placeholders for the job args.
    - Single job options file connecting jobs and submit scripts and listing
      all arguments for each job.
    - Single dagman config file.
    - Single shell script to run at submitter that submits the jobs.
    ...
    def create_job(self, job_exe, job_args, job_name, job_dir,
        ...
        - Everything is bundled into ``job_dir``
        - The jobfile is held simple, it only defines a ``/bin/bash`` call to
          the single bash script, which is also generated by this method and
          passes all the arguments. This has the advantage of being able to
          make calls pre and post of the actual job runnable.
          Each job is always calling the genrated bash scriopt via the given
          `bach_exe` and in the bash script the actual job command is called.
        - ``job_exe`` is a list of commands that are put in the generated
          bash script. The primary command that is executed always starts with
          ``job_exe[0] ... job_exe[N]``
        - After the job excecutables, the ``job_args`` dictionary is used to
          provide different arguments for each job to the bash script. These
          are written into the job list file from where DAGMan passes them to
          the submit script (generated here) which passes them to the bash
          script, like
          ``job_exe[0] ... job_exe[N] --key1=arg1[i] ... --keyM=argM[i]``
          for each job.
        - ``jobname`` is used to name the auto-generated job and shell files in
          a consistent way.
        - ``job_setup_pre`` and ``job_setup_post`` can be used to provide shell
          commands that are equal for each jobs and are placed above (``pre``)
          and below (``post``) the actual command line in the generated bash
          script.
        ...
        # Create and write the job, submitter and infrastructure files
        _sn_sub = self._write_submit_scripts(
            job_name, job_dir, extra_sub_args, bash_exe, job_args)
        _sn_sh = self._write_job_shell_scripts(
            job_exe, job_name, job_dir, job_setup_pre, job_setup_post)

        _sn_job = self._write_job_overview(
            job_name, job_dir, job_args, ram, njobs)
        _sn_cfg = self._write_dagman_config(job_name, job_dir)
        _sn_start = self._write_start_script(job_name, job_dir)

        if verbose:
            print("Generated files:")
            print("- 1x condor submit script '{}'".format(_sn_start))
            print("- 1x job argument list '{}'".format(_sn_job))
            print("- 1x job submit script '{}'".format(_sn_sub))
            print("- 1x job shell script '{}'".format(_sn_sh))
            print("- 1x dagman config '{}'".format(_sn_cfg))
            print("Now login to your submit node and run")
            print("  `{}`".format(os.path.join(job_dir, _sn_start)))
    ...
    def _write_submit_scripts(
            self, job_name, job_dir, extra_sub_args, bash_exe, job_args):
        """
        Write the submit script
        """
        job_fname = os.path.join(job_dir, job_name)

        s = ["processname    = {}".format(job_name)]
        s.append("executable     = {}".format(bash_exe))
        s.append("getenv         = True")
        ...
        s.append("request_memory = $({})".format(self._VAR_INT_RAM))
    ...
    def _write_job_shell_scripts(
        ...
        s.append("# Actual main job command")
        s.append("echo '----- Script output -----'")
        # Just pass all arguments from the submit script to the callable
        s.append(" ".join(job_exe) + ' "$@"')
        s.append("echo '----- !Script output -----'")
        s.append("echo")
    ...
    def _write_job_overview(self, job_name, job_dir, job_args, ram, njobs):
        """
        Write list that includes all submit files and jobnames.
        """
        s = []
